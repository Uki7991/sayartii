<template>
    <div class="bg-gray-50 font-roboto">
        <header class="py-3 shadow-lg border-b border-gray-900 fixed top-0 z-10 w-full bg-gray-50">
            <div class="container mx-auto flex justify-between items-center px-1 md:px-0">
                <inertia-link href="/" class="w-auto">
                    <svg class="h-7 md:h-11 w-auto" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 694 391">
                        ...
                        <g fill="none" fill-rule="evenodd">
                            <g fill-rule="nonzero">
                                ...
                                <path fill="#F4327F"
                                      d="M152.2 153a27 27 0 0119.1 33l-28 104.4a27 27 0 11-52.1-14l28-104.3a27 27 0 0133-19.1zm-72 0a27 27 0 0119.1 33l-28 104.4a27 27 0 11-52.1-14l28-104.3a27 27 0 0133-19.1zM104 376.4a27 27 0 110-54 27 27 0 010 54zm-72 0a27 27 0 110-54 27 27 0 010 54z"></path>
                            </g>
                            <path fill="#333"
                ...
                <div class="flex w-1/2">
                    <div @click="openFilters"
                         class="bg-pink-500 flex md:space-x-1 px-1 items-center text-white rounded-l md:px-5 cursor-pointer">
                        <span class="text-base hidden md:block">Filters</span>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                             class="fill-current text-white">
                            <path
                                d="M13 5H22V7H13zM2 7L9 7 9 9 11 9 11 3 9 3 9 5 2 5zM9 17H22V19H9zM19 11H22V13H19zM17 15L17 9.012 15 9.012 15 11 2 11 2 13 15 13 15 15zM7 21L7 15 5 15 5 17 2 17 2 19 5 19 5 21z"></path>
                        </svg>
                    </div>
                ...
                <div class="flex md:space-x-6 space-x-2">
                    ...
                    <button @click="openSidebar"
                        ...
                        <img src="/assets/icons/stats.svg" class="md:hidden w-8 h-8" alt="">
                    </button>
                </div>
            </div>
        </header>
        ...
        <div @click="openSellSidebar"
             class="animate-ping fixed bottom-8 right-8 md:bottom-16 md:right-16 rounded-full bg-pink-600 w-16 h-16 flex items-center justify-center text-white cursor-pointer">
        </div>
        <div @click="openSellSidebar"
             class="fixed bottom-8 right-8 md:bottom-16 md:right-16 rounded-full bg-pink-600 w-16 h-16 flex items-center justify-center text-white cursor-pointer">
            Sell
        </div>
        ...
        <modular-sidebar title="Sell your car" classes="rounded-bl-md w-2/6 min-h-full" @close="closeSellSidebar"
                         :show="sellSidebar">
            <div class="px-8 overflow-y-auto">
                ...
                <div class="py-1">
                    ...
                    <select-with-search
                        class="flex flex-col items-center border rounded-md px-3 border-gray-400 w-full bg-white"
                        :collection="cars" v-model="formCreate.model"></select-with-search>
                    <p class="text-xs text-red-600 mb-2" v-if="$page.props.errors.model">
                        {{$page.props.errors.model}}</p>
                </div>
                <div class="py-1">
                    <label for="year">Year <span class="text-red-600">*</span></label>
                    <input type="number" id="year" v-model="formCreate.year"
                ...
                <div class="py-1">
                    ...
                    <input type="number" v-model="formCreate.price" id="price"
                           class="w-full rounded-md focus:ring-black focus:border-black">
                    <p class="text-xs text-red-600 mb-2" v-if="$page.props.errors.price">
                        {{$page.props.errors.price}}</p>
                </div>
                <div class="py-1">
                    <label for="mileage">Mileage <span class="text-red-600">*</span></label>
                    <input type="number" v-model="formCreate.mileage" id="mileage"
                           class="w-full rounded-md focus:ring-black focus:border-black">
                    <p class="text-xs text-red-600 mb-2" v-if="$page.props.errors.mileage">
                        {{$page.props.errors.mileage}}</p>
                </div>
                <div class="py-1">
                    <label for="phone">Phone <span class="text-red-600">*</span></label>
                    <input type="number" v-model="formCreate.phone" id="phone"
                           class="w-full rounded-md focus:ring-black focus:border-black">
                    <p class="text-xs text-red-600 mb-2" v-if="$page.props.errors.phone">
                        {{$page.props.errors.phone}}</p>
                </div>
                <div class="py-1">
                    <label for="whatsapp">Whatsapp <span class="text-red-600">*</span></label>
                    <input type="number" v-model="formCreate.whatsapp" id="whatsapp"
                           class="w-full rounded-md focus:ring-black focus:border-black">
                    <p class="text-xs text-red-600 mb-2" v-if="$page.props.errors.whatsapp">
                        {{$page.props.errors.whatsapp}}</p>
                </div>
                <div class="py-1">
                    <label for="title">Title <span class="text-red-600">*</span></label>
                    <input type="text" id="title" v-model="formCreate.title"
                           class="w-full rounded-md focus:ring-black focus:border-black">
                    <p class="text-xs text-red-600 mb-2" v-if="$page.props.errors.title">
                        {{$page.props.errors.title}}</p>
                </div>
                <div class="py-1">
                    <label for="description">Description <span class="text-red-600">*</span></label>
                    <textarea id="description" rows="4" v-model="formCreate.description"
                              class="w-full rounded-md focus:ring-black focus:border-black"></textarea>
                    <p class="text-xs text-red-600 mb-2" v-if="$page.props.errors.description">
                        {{$page.props.errors.description}}</p>
                </div>
                <div v-for="(item, index) in attributes" :key="index+'Create'" class="py-1"
                     :class="[index, attributes]">
                    ...
                    <div class="flex flex-wrap">
                        <check-box v-if="item.type === 'checkbox'" class="flex-initial mr-2 mb-3" :class="indexAttr"
                                   ...
                                   :name="index" v-model="formCreate.attributesArr[index][indexAttr]"
                                   :type="value"></check-box>
                        <radio-box v-if="item.type === 'radio'" class="flex-initial mr-2 mb-3"
                                   v-for="(value, indexAttr) in item.attributesArr" :key="indexAttr+'radio'"
                                   :name="index" v-model="formCreate.attributesArr[index]" :type="value"></radio-box>
                        <p class="text-xs text-red-600 mb-2 w-full" v-if="$page.props.errors['attributesArr.'+index]">
                            {{$page.props.errors['attributesArr.'+index]}}</p>
                    </div>
                </div>
            </div>
            <template #footer>
                <button :disabled="disabledButton" @click="submitFormCreate"
                        class="bg-pink-600 py-4 flex items-center justify-center text-white text-lg font-medium mt-auto cursor-pointer focus:outline-none">
                    Create
                </button>
            </template>
        </modular-sidebar>
            ...
            return {
                server: {
                    process: (fieldName, file, metadata, load, error, progress, abort, transfer, options) => {
                        ...
                        request.open('POST', route('images.post-image', {
                            dir: 'announcements',
